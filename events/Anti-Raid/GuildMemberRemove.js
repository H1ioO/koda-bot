...
module.exports = async (db) => {
    ...
    const auditLogs = await (await db.guild).fetchAuditLogs({ type: AuditLogEvent.MemberKick, limit: 1 });
    ...
    if (executor) {
        ...
        if (await db.get(`ban_${(await db.guild).id}`) === true && !perm) {
            ...
            if (punish === "ban") {
                const rsl = await executor.ban({ reason: `Antiraid | Expulsion de membre` }).catch(() => { });
                if (rsl) user_punish = true;
            } else if (punish === "kick") {
                const rsl = await executor.kick(`Antiraid | Expulsion de membre`).catch(() => { });
                if (rsl) user_punish = true;
            } else if (punish === "derank") {
                const rsl = await executor.roles.set([], `Antiraid | Expulsion de membre`).catch(() => { });
                if (rsl) user_punish = true;
            }
