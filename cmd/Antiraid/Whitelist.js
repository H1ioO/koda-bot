...
module.exports = {
    name: 'whitelist',
    aliases: ["wl"],
    description: "Gestion de la whitelist antiraid",
    go: async (client, db, message, args, prefix, color) => {
        if (await db.get(`owner_${client.user.id}_${message.author.id}`) === true) {
            if (args[0] === "add") {
                ...
                if (await db.get(`wl_${message.guild.id}_${member.id}`) === true) return message.reply({ allowedMentions: { repliedUser: false }, content: `${member.username} est déjà whitelist` });
                ...
                if (message.mentions.members.first()) member = client.users.cache.get(message.mentions.members.first().id);
                if (!member) return message.reply({ allowedMentions: { repliedUser: false }, content: `Aucun membre trouvé pour \`${args[1] || "❌"}\`` });
                if (await db.get(`wl_${message.guild.id}_${member.id}`) === null) return message.reply({ allowedMentions: { repliedUser: false }, content: `${member.username} n'est pas whitelist` });
                ...
                const data = (await db.all()).filter(data => data.ID.startsWith(`wl_${message.guild.id}`)).sort((a, b) => b.data - a.data);
                const count = 15;
                let p0 = 0;
                let p1 = count;
                let page = 1;
                ...
                let embed = new EmbedBuilder()
                    ...
                    .setFooter({ text: `${page} / ${Math.ceil(data.length / count) === 0 ? 1 : Math.ceil(data.length / count)}`, iconURL: message.author.displayAvatarURL() })
                    .setColor(color)
                    .setDescription(data
                ...
                if (data.length > count) {
                    ...
                    const collector = msg.createMessageComponentCollector({ componentType: ComponentType.Button, time: 60000 * 5 });
                    collector.on("collect", async interaction => {
                        ...
                        if (interaction.customId === `wl1_${message.id}`) {
                            ...
                            embed.setFooter({ text: `${page} / ${Math.ceil(data.length / count) === 0 ? 1 : Math.ceil(data.length / count)}`, iconURL: message.author.displayAvatarURL() })
                                .setDescription(data
                                    .slice(p0, p1).map((m, c) => `<@${m.ID.split("_")[2]}> `).join("\n") || "Aucune donnée trouvée");
